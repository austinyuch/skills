...
### 2. TDD as EDD's Code-Based Grader
TDD is the underlying implementation engine for Eval-Driven Development (EDD) (see `eval-harness` skill).
- While EDD defines the macro-level **Capabilities** (what the AI must achieve) and **Regressions** (what must not break), TDD provides the micro-level **Code-Based Graders** (unit/integration test scripts).
- Your TDD test commands (e.g., `npm test`) will directly become the execution scripts for the EDD evaluation reports.
