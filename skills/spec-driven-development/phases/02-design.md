...
4.2 **(新增) 架構設計深度 (Architecture Design Depth)**：完整方法論見 [`references/architecture-design-depth.md`](../references/architecture-design-depth.md) 的 **Part A**。
   - **實作方案比較（條件式啟動）**：僅當設計屬架構顯著 / 非 trivial（>8 檔案、新服務、新抽象、跨模組契約變更、或選型本身即風險）時，產出 2–3 個明顯不同的方案（至少含 minimal-viable 與 ideal-architecture，可加 creative），各附 Effort/Risk/Pros/Cons/Reuses，收尾 RECOMMENDATION，並以單一決策 **STOP** 等使用者批准後才定稿。小改動跳過（KISS/YAGNI）。
...
4.4 **(新增) Compliance Inventory Design Hook**：若 Phase 1 觸發 `iso-ai-security-auditor`，必須把其 control / legal / log / AI governance inventory 轉成設計期的 evidence boundary，而不是只放在附錄。
   - 在 `design.md` 的 **System Architecture Documentation Impact** 或等價章節納入 AI component boundary、PII/data flow、log/telemetry flow、trust boundary、external-control owner、evidence refs、以及 missing-evidence items。
...
24. 與前端設計相關任務請參考 skills: `brand-guideline-company`, `ui-skill`, `frontend-design-skill`, `frontend-patterns`, `theme-factory`
