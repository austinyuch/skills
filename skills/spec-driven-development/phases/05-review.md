...
- **架構深度評估 (Architecture-Depth Review)**：完整 checklist 見 [`references/architecture-design-depth.md`](../references/architecture-design-depth.md) 的 **Part B**。逐項評估（零發現也要寫「無問題」）：耦合 / 依賴圖（before/after）、資料流四路徑（happy/nil/empty/error）、state machine 合法與被擋的非法轉移、每個新整合點的 production 失敗情境、rollback posture、scaling / SPOF、observability-as-scope。並對新增 module/wrapper/抽象套用 **deletion test**（刪掉它會集中還是只搬移複雜度？）與 shallow-module smell；併入「設計一致性」與「程式碼品質」分析，不另設權重。
...
- **System Architecture Documentation Review**：若本次為 project start、cross-module/system design、shared contract/runtime topology 變更、或 stakeholder-facing project review 前置工作，必須讀取 `../references/system-architecture-lifecycle.md`，並檢查 `docs/architecture.md` / `docs/architecture/index.html`（或 repo 既有等價路徑）是否存在、是否 current、是否與 `design.md` / `review.md` 一致、是否把 planned/mock-backed/historical state 寫成 executed truth。
...
- **Conflict Handling Evidence**：若本次工作曾遇到 concurrent writable lane 風險，必須依 `../../shared-governance/references/conflict-evidence-review-checklist.md` 檢查 downgrade / reconcile / ownership resolution evidence，而不是靠「最後沒有出錯」當成通過理由
...
- **System Architecture Documentation**：architecture doc state（`current` / `needs_update` / `missing_but_required` / `not_yet_required` / `stale_or_overclaiming`）、markdown source、HTML presentation、SAA good parts scope、freshness evidence、以及是否需要 follow-up。
...
- **問題清單**：發現的所有問題，按嚴重程度分類。每條 finding 採 **confidence + verbatim-quote 防誤報格式**（見 `references/architecture-design-depth.md` Part B3）：格式 `[SEVERITY] (confidence: N/10) file:line — 描述`；提報前**必須能逐字引用觸發該 finding 的原始程式碼行**，引用不出來就把 confidence 降到 4–5 並壓到附錄。confidence 是「這條 finding 有多可信」，與 0–10 維度評分是不同軸、兩者並存。
- **改進建議**：具體的優化建議和修改方案
- **優化優先級**：建議的修復順序
