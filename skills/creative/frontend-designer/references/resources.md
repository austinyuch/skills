...
所有路徑以 `~`（使用者家目錄）表示的本機絕對路徑。這些是**外部專案 / 資源**，本 skill 讀取 / 複製 / 呼叫它們，不修改它們。

## open-design（design tokens + craft 準則 + 版型）— 最重要的設計資源

Canonical repo：`~/projects/open-design/`（`aclab-open-design` 只是 build 產物，忽略）。**用 file-based 消費，不要跑那個 app。**

- **品牌 token 層（152 套）**：`~/projects/open-design/design-systems/<name>/`
...
- **通用 craft 準則（brand-agnostic，12 份）**：`~/projects/open-design/craft/*.md`
...
- **可 render 版型（111 套）**：`~/projects/open-design/design-templates/<shape>/`
...
Repo：`~/projects/gstack/`。CLI/skill 工具箱，**framework-agnostic、token-driven**，render 用 **Pretext**（非 A2UI）。

- `~/projects/gstack/DESIGN.md` — canonical design-system 範例 + 每個專案該產出的 exact schema（aesthetic/type/color/spacing/layout/motion/grain + Decisions Log）
- `~/projects/gstack/design-consultation/sections/proposal-and-preview.md` — **最richest**：DESIGN.md template、font blacklist、aesthetic/color/motion taxonomy、AI-slop 清單
- `~/projects/gstack/design-consultation/SKILL.md` — design-system 建立流程（propose-don't-menu、coherence 規則）
- `~/projects/gstack/design-html/SKILL.md` — production HTML 慣例、Pretext API cheatsheet、"Always/Never include" 清單
- `~/projects/gstack/design-html/vendor/pretext.js` — 30KB 零依賴 render 引擎（inline 或 `https://esm.sh/@chenglou/pretext`）
- `~/projects/gstack/review/design-checklist.md` — **grep 得到的前端 anti-pattern 複核清單**（confidence tier + AUTO-FIX/ASK 分類）→ Phase 3 直接用
...
- 協定 / spec：`~/projects/openclaw/vendor/a2ui/README.md`、`.../specification/0.9/json/*.json`、範例 `.../0.9/json/contact_form_example.jsonl`
- Lit renderer（standard catalog）：`~/projects/openclaw/vendor/a2ui/renderers/lit/`（`@a2ui/lit`）；host `~/projects/openclaw/src/canvas-host/a2ui/index.html`；bundle `pnpm canvas:a2ui:bundle`（`scripts/bundle-a2ui.sh`）；render 呼叫 `window.openclawA2UI.applyMessages(msgs)`
- **React/shadcn renderer（Giant v0.10，A2UI 元件名 → shadcn resolver）**：
  - catalog：`~/projects/giant-template-orchestrator/configs/a2ui/giant_catalog.json`（Container/Text/Image/ChartWidget/DataGrid/HeroBanner/Section/ColumnsLayout + `stylePreset` 品牌 + functions `formatTWD`…）
  - renderer：`~/projects/giant-template-orchestrator/frontend/lib/a2ui/{renderer.ts,catalog.ts,binding.ts}`；render 站點 `frontend/app/agent/page.tsx`（`new A2UIRenderer().process(msgs)` → `renderSurface(id)`）
  - Go composer（fluent）：`~/projects/giant-template-orchestrator/internal/adkagent/a2ui/{builder.go,types.go}`（`NewSurface(id, GiantCatalogID).SetData(...).AddComponents(...).Build()`）
- 輕量 CMS 方言（非 Google wire format）：`~/projects/go-cms-ai-site/skills/a2ui-protocol/SKILL.md`（`uiType`/`targetMutation`/`mutationVariables`）
...
Repo：`~/projects/aclab-uat-demo-agent/`。Go `uatdemo` binary（headless Chromium via `playwright-go`）。

- 打包好的 binary（免 build）：`~/projects/aclab-uat-demo-agent/skills/uat-demo-agent/scripts/uatdemo-{linux-amd64,linux-arm64,darwin-amd64,darwin-arm64,windows-amd64.exe}`（+ `uatdemo.py` wrapper）
