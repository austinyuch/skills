...
2026-07-07 retrospective：不要讓 print-friendly tunnel vision 摧毀 screen UI。GTO 的真實教訓是「本日修改前的視覺正常，只是 print 機制沒作用」；錯誤做法是重寫成 print-friendly 白板文件。以後遇到既有 showcase，第一步是找修改前 baseline 並截圖觀摩，第二步才是補 print。contract pass 不是美觀驗收。
...
1. **既有 screen template 有效**：先用 `git log -- docs/showcase` 找到今日修改前或最近 committed 版本，使用 `git archive` / `git show` 抽出 baseline，截 1440px 與 mobile screen。若 baseline 有清楚的 hero、產品截圖、stats/proof、cards、CTA 與品牌視覺，只允許 additive print patch。
2. **既有 screen template 失效或使用者明確要求 full regeneration**：使用 `assets/showcase-template-screen-first.html`、`assets/showcase-template-screen-first.css`、`assets/showcase-template-screen-first.js` 作為起點。這組 template 由 GTO showcase 泛化而來，保留 dark brand hero、真實產品截圖、stats strip、pain/value/proof cards、manual/review resource links、final CTA、雙語 toggle 與 print control。
3. **reference snippet 不是 template**：本檔的 nav / JS / print CSS 片段是 patch pattern，用來補洞；不能拿來覆蓋整頁。
4. **驗收不是只跑 script**：`check_showcase_contract.py` 只能擋 print wiring / bilingual / contrast contract；screen UI 必須用 screenshot 目視確認，PDF 必須匯出目視確認。
5. **Chrome portal-safe first render**：如果交付對象可能用 Chrome/Chromium 從檔案管理器雙擊 HTML，`index.html` 必須能在 `file:///run/user/1000/doc/...` 類 document-portal 單檔路徑下正常顯示。保留 `style.css` / `showcase.js` 作為維護來源，但交付的 `index.html` 內嵌 CSS、JS、以及首屏/展示用 screenshots；用「只複製 index.html 到 `/tmp` 後截圖」驗證。
...
單檔情境（要能單獨寄出 / 內嵌）也可把 CSS / JS inline 進 `index.html`；此時 print 區塊就放在同一個 `<style>` 內。若使用者可能用 Chrome 雙擊開檔，單檔情境不是 optional，而是交付要求：`index.html` 需 inline CSS/JS/必要圖片；`style.css` / `showcase.js` 繼續保留以便維護。

若要從本 skill template 起稿：

```bash
cp ~/.config/opencode/skills/social-media/marketing-showcase-creator/assets/showcase-template-screen-first.html docs/showcase/index.html
cp ~/.config/opencode/skills/social-media/marketing-showcase-creator/assets/showcase-template-screen-first.css docs/showcase/style.css
cp ~/.config/opencode/skills/social-media/marketing-showcase-creator/assets/showcase-template-screen-first.js docs/showcase/showcase.js
```

複製後再把 `{{PLACEHOLDER}}` 全部替換成專案文案與真實 asset path；不要留下 template token。
...
1. **Screen baseline**：修既有 showcase 前，先用 `git log --date=iso-local -- docs/showcase` 找出最近可信版本，再用 `git archive <rev> docs/showcase` 抽到 `/tmp` 觀摩。接著用 `webapp-testing` / `frontend-designer` 的 Playwright screenshot pattern 對 screen 版在 390 / 768 / 1440 取 baseline（靜態 HTML 可直接 `file://`；動態 app 先走 local runtime governance）。保存到 `temp/showcase-print-visual/baseline/`。
