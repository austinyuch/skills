...
- 具體的建立 / 重用 / 清理規則，必須讀取 `shared-governance` skill 的 `references/git-worktree-guide.md`；branch namespace、layout、命令範本與 handoff checklist 則讀取 `shared-governance` skill 的 `references/git-worktree-templates.md`；多 lane 衝突處理則讀取 `shared-governance` skill 的 `references/concurrent-writable-lanes.md` 與 `shared-governance` skill 的 `references/pre-write-conflict-checklist.md`。
...
- 風險代碼命名請與共用 taxonomy 對齊：`../../docs/DEMO_RISK_WARNING_TAXONOMY.md`。
...
若 `code-review` skill 家族（`code-review` 本體與其 depth-review 同伴 `test-quality-reviewer`、`code-refactoring-advisor`、`test-design-generator`、`security-risk-reviewer`、`sonarqube-bridge`）**對當前 agent 可用**，SDD 應在對應階段把它們當成 **review / test 的輔助輸入與 left-shift 偵測層**，但必須遵守 capability gate 與 authority 邊界：

- **Availability 的判定範圍**：「可用」**不限於當前 workspace 是否 vendor 了這些 skill**。這個家族由其專案的 `code-review-publish` 流程發布到 global skill homes（`~/.claude/skills`、`~/.config/opencode/skills`、`~/.kiro/skills`、`~/.codex/skills`、`~/.gemini/antigravity/skills`），因此一旦發布，它在該機器上的 **每個 workspace 都可用**。判定順序：workspace-local skill → global skill home → 對 `code-review` 而言還包含已發布 bundle 內的 `review-cli-<os>-<arch>` binary（PATH 或 bundle `scripts/`）。任一來源命中即視為可用。
...
- **Phase 2 Design**：`code-review` 的 `impact` / `bounded-context` / `search-code --graph-only` 可佐證 `[Impacts: …]` 宣告與 lightweight FMEA 的 blast-radius（optional, recommended）。
- **Phase 3 Tasks**：`test-design-generator` 產生 boundary / equivalence / pairwise 候選案例，轉成 test obligations 與 `TESTS.md` 草稿列（advisory）。
- **Phase 4 Implementation（REFACTOR）**：`code-refactoring-advisor`、`test-quality-reviewer`、以及對 changed files 跑 `code-review`（含 `review --vet --race` 等背景經典問題偵測）為 left-shift（conditionally required，依語言 gate）。
- **Phase 5 Review**：`code-review` 與 `test-quality-reviewer` 為 required inputs（非 verdict）；`security-risk-reviewer` pre-flight 餵 security-review gate；若組織採用 SonarQube，`sonarqube-bridge` 為 optional enrichment。
