...
- 若任務會跨 `TESTS.md` / `SPECS.md` / `RTM.md` 進行摘要寫回，必須依 `shared-governance` skill 的 `references/cross-artifact-regeneration-order.md` 明確指定 regenerate 順序，不得在任務中留下含糊的「之後再同步」描述。
