...
**跨 spec 的系統架構文件生命週期**（markdown + HTML 雙重文件、SAA good parts、project review claim-cap、與 spec lifecycle reconciliation）見：[references/system-architecture-lifecycle.md](./references/system-architecture-lifecycle.md)。當 project start、Phase 2 design、Phase 5 review、或 project-review artifact 需要能跟人溝通的整體架構觀點時讀取；它不取代 branch-spec `design.md`，只定義如何把 accepted design evidence 維持成 current architecture snapshot。
