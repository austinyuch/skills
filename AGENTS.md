...
- **Native binaries / models are gitignored, never git LFS.** `code-review` needs `review-cli-<os>-<arch>`
  ...
  - The 6 binaries are published as release **`review-cli-v0.16.1`**; installers fetch the matching one
    ...
    **private**, so this needs `gh` auth. To cut a new build: `gh release create review-cli-vX.Y.Z
    ...
    (`review-cli-v…`) in `bin/aclab-skills.mjs`, `bin/aclab_skills.py`, `scripts/install.sh`,
    `scripts/install.ps1`.
